...
`Configuration` redacts the access token in `Debug`. Hosted port URLs are redacted in `HostPortResponse`’s `Debug`. Commands are **never** auto-retried. Pass `Idempotency-Key` via `create_with_idempotency`.
